...
func (m BlogModel) Get(subdomain string) (*Blog, *BlogPost, error) {
	...
	err := m.DB.QueryRow(query, subdomain).Scan(&blog.Name, &blog.Subdomain, &blog.UserID, &blogPost.ID, &blog.UpdatedAt)
	if err != nil {
	...
	err = m.DB.QueryRow(query, blogPost.ID).Scan(&blogPost.Title, &blogPost.Content, &blog.CreatedAt, &blogPost.UpdatedAt)
	if err != nil {
